model (
  name prod.listing_protocol_totals_unique,
  kind view
);
...
from wallets.prod.listing_protocol_totals_unique_daily
where load_dt = (select max(load_dt) from wallets.prod.listing_protocol_totals_unique_daily);
