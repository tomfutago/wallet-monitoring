select 1 as id, 'Essential Cover Exposure' as name, format('${:,.2f}', sum(usd_protocol_exposed)) as usd_value, format('Ξ{:,.2f}', sum(eth_protocol_exposed)) as eth_value
from prod.cover_wallet_protocol_totals where listing = 'Essential Cover'
union all
select 2, 'Coverage Ratio 1>', format('${:,.2f}', sum(usd_protocol_exposed)), format('Ξ{:,.2f}', sum(eth_protocol_exposed))
from prod.cover_wallet_protocol_totals where listing = 'Essential Cover' and coverage_cover_ratio > 1
union all
